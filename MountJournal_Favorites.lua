...
function MountFavorites_Mount()
    ...
    if IsModifiedClick("CHATLINK") then
        ...
        primary = GROUND
        -- mountIDs = MountFavorites_GetFavoriteMountIDs(primary)
    end
    
    local mountIDs = {}
    local favs = MountFavorites_GetFavoriteMountIDs()
    local avail = LibMounts:GetMountList(primary)
    for id, spell in pairs(favs) do
        if avail[id] ~= nil then
            table.insert(mountIDs, spell)
        end
    end
